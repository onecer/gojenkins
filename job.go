...
func (j *Job) GetBuild(ctx context.Context, id int64) (*Build, error) {
	...
	jobURL := fmt.Sprintf("/job/%s", j.GetName())
	// fmt.Printf("jobURL: %s j.Raw.URL: %s j.Jenkins.Server: %s j.Jenkins.Requester.Base: %s \n", jobURL, j.Raw.URL, j.Jenkins.Server, j.Jenkins.Requester.Base)
